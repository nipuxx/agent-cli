...
def _create_job(
    *, objective: str, title: str | None = None, kind: str = "generic", cadence: str | None = None
    ...
    try:
        ...
        plan = _initial_plan_for_objective(objective)
...
def _initial_plan_for_objective(objective: str) -> dict[str, Any]:
...
def _format_initial_plan(plan: dict[str, Any]) -> str:
...
def _minimal_live_event_line(event: dict[str, Any], *, chars: int = 92) -> str:
    kind = str(event.get("event_type") or "")
    title = str(event.get("title") or "").strip()
    body = _generic_display_text(event.get("body") or "")
    metadata = event.get("metadata") if isinstance(event.get("metadata"), dict) else {}
...
def _tool_live_summary(tool: str, metadata: dict[str, Any], body: str) -> str:
    args = _event_tool_args(metadata)
    clean_body = _clean_step_summary(body)
    if tool == "web_search":
...
def _event_tool_args(metadata: dict[str, Any]) -> dict[str, Any]:
...
def _generic_display_text(value: Any) -> str:
